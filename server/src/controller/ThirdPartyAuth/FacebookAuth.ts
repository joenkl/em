...
export class FacebookAuth implements IThirdPartyAuth {
    private IUser: IUser; 
    ...
    login = async (profile): Promise<IUserModel> => {
        ...
        let user: IUserModel | undefined = await this.IUser.findUserByFacebookID(authID);
        if (!user) {
            user = await this.IUser.findUserByEmail(_.get(profile, 'emails[0].value', ''));

            if (!user) {
                const newUser: IUserModel = new CUserModel();
